...
public class FileController {
	...
	public ModelAndView mainPage() {
		...
		model.addObject("userFileList", fileUploadDao.fileList(
	...
	public ModelAndView upload(@RequestParam("file") MultipartFile[] files) throws IOException {
		...
		for (MultipartFile file : files) {
			...
			if (!file.getOriginalFilename().isEmpty()) {
				if (!fileUploadDao.fileUpload(SecurityContextHolder.getContext().getAuthentication().getName(), file)) {
					duplicate = true;
				}
			} else {
	...
	public String delete(@PathVariable int fileId) {
		fileUploadDao.fileDelete(fileId, SecurityContextHolder.getContext().getAuthentication().getName());
		return "redirect:/";
	}
	
	@RequestMapping(value = "/download/{fileId}", method = RequestMethod.GET)
	public String download(HttpSession session, HttpServletResponse response, @PathVariable int fileId) throws Exception {
		try {
			String fileName = fileUploadDao.targetFileName(fileId);
